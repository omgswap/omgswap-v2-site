...
interacts with the OMGSwap Protocol using one specific set of tools (`truffle` + `npm` + `mocha`).
...
In order to reference the OMGSwap V2 contracts, you should use the NPM artifacts we deploy containing the core and
...
[`@OMGSwap/v2-core`](https://www.npmjs.com/package/@OMGSwap/v2-core) and
[`@OMGSwap/v2-periphery`](https://www.npmjs.com/package/@OMGSwap/v2-periphery) packages.

```shell script
npm i --save @OMGSwap/v2-core
npm i --save @OMGSwap/v2-periphery
```

If you check the `node_modules/@OMGSwap` directory, you can now find the OMGSwap V2 contracts.

```shell script
moody@MacBook-Pro ~/I/u/demo> ls node_modules/@OMGSwap/v2-core/contracts
OMGSwapV2ERC20.sol    OMGSwapV2Pair.sol     libraries/
OMGSwapV2Factory.sol  interfaces/           test/
moody@MacBook-Pro ~/I/u/demo> ls node_modules/@OMGSwap/v2-periphery/contracts/
OMGSwapV2Migrator.sol  examples/              test/
OMGSwapV2Router01.sol  interfaces/
OMGSwapV2Router02.sol  libraries/
```
...
Now let's start with the constructor. You need to know where the `OMGSwapV2Factory` is deployed in order to compute the
...
The [`OMGSwapV2Library`](/docs/v2/smart-contracts/library/) has some helpful methods for this.

```solidity
pragma solidity =0.6.6;

import '@OMGSwap/v2-periphery/contracts/libraries/OMGSwapV2Library.sol';
import '@OMGSwap/v2-core/contracts/interfaces/IOMGSwapV2Pair.sol';

contract LiquidityValueCalculator is ILiquidityValueCalculator {
    function pairInfo(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB, uint totalSupply) {
        IOMGSwapV2Pair pair = IOMGSwapV2Pair(OMGSwapV2Library.pairFor(factory, tokenA, tokenB));
...
Note you should only deploy the precompiled OMGSwap contracts in the `build` directories for unit tests.
...
This is problematic because in OMGSwap V2 we use the hash of the bytecode in the v2-periphery
[`OMGSwapV2Library`](https://github.com/OMGSwap/OMGSwap-v2-periphery/blob/master/contracts/libraries/OMGSwapV2Library.sol#L24),
to compute the pair address.

To get the bytecode for deploying OMGSwapV2Factory, you can import the file via:

```javascript
const OMGSwapV2FactoryBytecode = require('@OMGSwap/v2-core/build/OMGSwapV2Factory.json').bytecode
```
