...
In this guide we will create a web interface that consumes and displays data from the OMGSwap Subgraph. The goal is to provide a quick overview of a setup that you can extend to create your own UIs and analytics around OMGSwap data.
...
```javascript
yarn create react-app OMGSwap-demo
cd OMGSwap-demo
...
We need to set up some middleware in order to make requests to the OMGSwap subgraph and receive data. To do this we'll use Apollo and create a graphql client to handle this.

1. Add the imports shown below and instantiate a new client instance. Notice how we use the link to the OMGSwap subgraph here.
...
export const client = new ApolloClient({
 link: new HttpLink({
   uri: "https://thegraph.com/explorer/subgraph/OMGSwap/OMGSwap-v2"
 }),
...
Next we'll construct our query and fetch data. For this example we will fetch some data about the Dai token on OMGSwap V2. We'll get the current price, and total liquidity across all pairs. We'll be using the Dai address as an id in this query. We'll also fetch the USD price of ETH to help create USD conversion for Dai data.
...
Now we're ready to use these queries to fetch data from the OMGSwap V2 subgraph. To do this we can use the `useQuery` hook which uses our client instance to fetch data, and gives us live info about the status of the request. To do this add the following to your `App.js` file:
...
This should render a very basic page with these two stats about the Dai token within OMGSwap. This is a very basic example of what you can do with the OMGSwap subgraph and we encourage you to build out more complex and interesting tools!

You can visit our [analytics site](https://OMGSwap.info/) to see a more advanced analytics page, or visit [the github](https://github.com/OMGSwap/OMGSwap-info) for more detailed examples of using the OMGSwap subgraph to create UIs.
...
export const client = new ApolloClient({
  link: new HttpLink({
    uri: 'https://api.thegraph.com/subgraphs/name/OMGSwap/OMGSwap-v2'
  }),
  fetchOptions: {
    mode: 'no-cors'
  },
